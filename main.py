...
def download_video_with_progress(direct_link, output_path):
    ...
    with open(output_path, "wb") as f:
        if total_length is None:
            ...
            bytes_baixados = 0
            chunk_size = 8 * 1024 * 1024  # Aumentado para 8MB (melhor para arquivos grandes)
...
with col2:
    if st.button("📥 Baixar Vídeo 2 (TEVEO 1 - 6GB)", use_container_width=True):
        try:
            if os.path.exists(SAVE_PATH):
                os.remove(SAVE_PATH)
            st.info("Obtendo link do MediaFire (pode demorar)...")
...
if os.path.exists(SAVE_PATH):
    ...
    # Caminho correto para static files
    ...
    st.caption("💡 Use o mouse para avançar/retroceder. O player usa Range Requests (suporta arquivos grandes).")
